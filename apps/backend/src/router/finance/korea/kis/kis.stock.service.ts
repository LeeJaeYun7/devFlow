...
import { KisStock52WeekDto } from './dto/kis-stock.52week.dto';
...
export class KisStockService {
  ...
  async fetchFundamental(symbol: string): Promise<KisStockFundamentalDto | null> {
    ...
    try {
      ...
      this.logger.debug('KIS 응답 전체', data.output);
  ...
  async fetchTechnical(symbol: string): Promise<KisStockTechnicalDto | null> {
    ...
    try {
      ...
      this.logger.debug('KIS technical 응답 전체', data.output);
      ...
      const week52 = await this.fetchStock52Week(symbol);
      const high52Week = week52 ? +week52.high52Week : 0;
      const low52Week = week52 ? +week52.low52Week : 0;
  ...
  async fetchStock52Week(symbol: string): Promise<KisStock52WeekDto | null> {
    if (!symbol) {
      this.logger.warn('Symbol is required');
      return null;
    }
    ...
    try {
      ...
      const { data } = await firstValueFrom(this.http.get(url, { headers, params,  timeout: 5000 }));
      ...
      const o = data.output;
      return {
        high52Week: o.w52_hgpr,
        low52Week: o.w52_lwpr,
      };
    } catch (err) {
      this.logger.error(`[${symbol}] fetchStock52Week 오류: ${err}`);
      return null;
    }
  }
